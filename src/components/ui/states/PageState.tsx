...
export default function PageState({
    ...
    const fallbackDescription =
        resolvedStatus === 401
            ...
            : resolvedStatus === 403
              ? "คุณไม่มีสิทธิ์สำหรับการทำรายการนี้ สามารถกดขอสิทธิ์ใช้งานได้"
              : resolvedStatus === 409
    ...
    const handleRequestAccess = async () => {
        ...
        if (typeof navigator !== "undefined" && navigator.clipboard) {
            try {
                ...
                message.success("คัดลอกคำขอสิทธิ์แล้ว ส่งให้ผู้ดูแลระบบได้ทันที");
                return;
            } catch {
                // Fall through to hint message below.
            }
        }
    ...
    if (status === "error") {
        if (resolvedStatus === 401) {
            defaultAction = (
                <Space size={8}>
                    {onRetry ? <Button onClick={onRetry}>{t("page.retry")}</Button> : null}
